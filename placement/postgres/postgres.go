...
// Package postgres is placement.Store on pgx: what the paid archive requires
...
func (dialect) Name() string      { return "placement/postgres" }
...
func Open(ctx context.Context, dsn string) (*sqlstore.Store, error) {
	if dsn == "" {
		return nil, errors.New("placement/postgres: no DSN")
	}
	db, err := sql.Open("pgx", dsn)
	if err != nil {
		return nil, fmt.Errorf("placement/postgres: opening: %w", err)
	}
