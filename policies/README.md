...
namespaces, or verifying image signatures against a signing identity. None of
those are needed here yet. Image signature verification is the likely first
reason to bring one back — the publish pipeline already emits provenance and an
SBOM, and cosign verification would close that loop.
...
`policy-test.sh` submits one compliant manifest that must be admitted and seven
broken ones that must each be rejected with a specific message. A rule that
rejects everything is as broken as one that rejects nothing, which is why the
compliant case is in there.
