...
// Package sqlite is forge.Store on modernc.org/sqlite: what a one-node
...
func (dialect) Name() string           { return "forge/sqlite" }
...
func Open(ctx context.Context, path string) (*sqlstore.Store, error) {
	if path == "" {
		return nil, errors.New("forge/sqlite: no database path")
	}
	pragmas := []string{
	...
	if err != nil {
		return nil, fmt.Errorf("forge/sqlite: opening %s: %w", path, err)
	}
