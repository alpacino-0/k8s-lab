...
--
-- Identical to identity/sqlite/migrations/0001_init.sql apart from STRICT,
-- which is SQLite's way of making a TEXT column actually hold text; PostgreSQL
-- does that by default. Kept as a separate file rather than templated for the
-- same reason the evidence schemas are: a template would hide the next
-- divergence behind a conditional, and the point of the split is that the
