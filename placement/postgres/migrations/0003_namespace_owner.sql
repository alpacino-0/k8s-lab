...
-- column and anything written between then and now holds one. ON CONFLICT DO
-- NOTHING rather than a plain INSERT: two placements of the same tenant
-- legitimately share a namespace, and only the first of them is the claim.
...
INSERT INTO namespace_owner (namespace, tenant_id, claimed_at)
